...
import { completeTask, createList, createRedemption, createTask, deleteTask, type RewardEntry } from '../core'
...
const DATA: AccountData = { tasks: [DONE, TRASHED], lists: [WORK], entries: ENTRIES, redemptions: [TREAT] }
...
describe('reading a backup', () => {
  ...
  it('leaves out and counts the records it cannot read, keeping the rest (BAK-8)', () => {
    ...
    expect(read).toEqual({ data: { tasks: [DONE], lists: [], entries: [], redemptions: [TREAT] }, unreadable: 5 })
  })
})
