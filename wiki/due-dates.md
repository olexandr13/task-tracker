...
- **DUE-5** On a task row the date button sits before the repeat button, on show at rest whether
  set or not (UI-18). It is the icon alone, still tinted, the date being its name and tooltip; the
  date itself is spelled out **under the button**, at rest too (UI-27). A change is saved straight away.
...
- **DUE-9** It opens a panel with three quick choices — **Today**, **Tomorrow** and **Next week**,
  the Sunday that closes next week (weeks run Monday to Sunday) — each with its date spelled out beside it; a date field for any
...
clearing it), `src/core/due.ts` (which day a task is due, overdue, and the day Next week sets), `src/app/dueLabels.ts`
